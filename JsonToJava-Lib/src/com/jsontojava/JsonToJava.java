...
public class JsonToJava {
	...
	private boolean mShouldUseArrayInsteadofList = false;
	private boolean mShouldUseWrapperClasses = false;
	...
	public void fetchJson() {
		try {
			Object root = mJson != null? getJsonObject(mJson) : getJsonFromUrl(mUrl);
	...
	public void setShouldUseArrayInsteadofList(boolean shouldUseArrayInsteadofList) {
		this.mShouldUseArrayInsteadofList = shouldUseArrayInsteadofList;
	}

	public void setShouldUseWrapperClasses(boolean shouldUseWrapperClasses) {
		this.mShouldUseWrapperClasses = shouldUseWrapperClasses;
	}
...
//		System.out.println("\nFinished creating java classes.  Your files are located in " + zipFile.getAbsolutePath() );
	...
	public void outputClassFile(File directory) throws IOException{
		String fileSeparater = System.getProperty(PROPERTY_FILE_SEPARATOR);

		for (Map.Entry<String, NewType> entry : mTypes.entrySet()) {
			String className = entry.getKey();
			NewType type = entry.getValue();
			File file = new File(directory + fileSeparater + className + FILE_EXTENSION_JAVA);
...
//		BufferedReader reader = new BufferedReader()
	...
	private Object getJsonObject(String jsonString) throws IOException {
		Object retVal = new JSONObject();
		try {
			retVal = new JSONObject(jsonString);
		} catch (JSONException e) {
	...
	private Member generateMember(String key, Object current) {
		...
		if (current instanceof JSONArray) {
			memberBuilder.setShouldUseArrayInsteadOfList(mShouldUseArrayInsteadofList);
			...
			if (current instanceof JSONObject) {
				...
				String clazz = mShouldUseWrapperClasses ? TypeUtils.getPrimitiveClassType(current) : TypeUtils.getPrimitiveType(current);
	...
	private NewType generateClass(JSONObject obj, String typeName) {
		...
		if(keys != null){
			for (String s : keys) {
				...
				if ((current instanceof JSONArray) && !mShouldUseArrayInsteadofList) {
